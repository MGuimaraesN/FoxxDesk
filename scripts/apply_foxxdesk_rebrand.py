...
SCRIPT_VERSION = "v40-public-text-brand-workflow-safe-2026-09-06"
...
def run_icon_assets_v28(target: Path, args: argparse.Namespace, report: Dict[str, Any]) -> None:
    ...
    if completed.returncode != 0:
        report["pending"].append({"file": rel, "message": f"gerador de ícones falhou com exit {completed.returncode}; veja icon_assets_report.md e rebrand_v40.log"})
        return
...
def setup_logging_v28(target: Path, args: argparse.Namespace, report: Dict[str, Any]) -> None:  # type: ignore[override]
    """V36: mantém flags de logging e usa rebrand_v40.log por padrão."""
    log_path = Path(args.log_file).expanduser().resolve() if getattr(args, "log_file", None) else target / "rebrand_v40.log"
    log_path.parent.mkdir(parents=True, exist_ok=True)
    level = logging.DEBUG if getattr(args, "verbose", False) else logging.INFO
    root_logger = logging.getLogger()
    root_logger.handlers.clear()
...
# -----------------------------------------------------------------------------
# V40 - public text/vendor brand hardening
# -----------------------------------------------------------------------------
# Preserve internal protocol/API identifiers and legal upstream notices, but make
# every user-facing translation/value use the configured public brand.  This
# avoids broad identifier renames while preventing strings such as
# "Desenvolvido por RustDesk" from leaking into FoxxDesk builds.
_PRE_V40_PATCH_TEXT = patch_text
_PRE_V40_VALIDATE_BUILD_SAFETY = validate_build_safety

_LANG_TUPLE_RE_V40 = re.compile(
    r'(\(\s*"(?:[^"\\]|\\.)*"\s*,\s*")((?:[^"\\]|\\.)*)("\s*\))',
    re.S,
)


def _replace_public_brand_token_v40(value: str) -> str:
    return re.sub(r'(?i)rustdesk', lambda _m: APP_DISPLAY_NAME, value)


def patch_language_values_v40(rel: str, text: str) -> str:
    if rel == 'src/lang.rs':
        # Upstream intentionally excluded powered_by_me from custom-brand
        # substitution. FoxxDesk must not: it is a public attribution string.
        text = re.sub(r'\n\s*&&\s*name\s*!=\s*"powered_by_me"', '', text, count=1)
        return text
    if not (rel.startswith('src/lang/') and rel.endswith('.rs')):
        return text

    def repl(match: re.Match[str]) -> str:
        before, value, after = match.groups()
        return before + _replace_public_brand_token_v40(value) + after

    return _LANG_TUPLE_RE_V40.sub(repl, text)


def patch_public_vendor_and_literals_v40(rel: str, text: str) -> str:
    # Public Flutter title still hardcoded upstream in compatible UI mode.
    if rel == 'flutter/lib/desktop/widgets/tabbar_widget.dart':
        text = re.sub(
            r'(?m)(child:\s*const\s+Text\(\s*)"RustDesk"',
            lambda m: m.group(1) + f'"{APP_DISPLAY_NAME}"',
            text,
            count=1,
        )

    # About dialog / legacy Sciter UI copyright. Legal license files are not
    # touched here; only the app's own visible copyright field is rebranded.
    if rel in {'flutter/lib/desktop/pages/desktop_setting_page.dart', 'src/ui/index.tis'}:
        text = re.sub(r'Purslane Tech Pte\. Ltd\.?', COPYRIGHT_OWNER, text)
        text = re.sub(r'Purslane Ltd\.?', COPYRIGHT_OWNER, text)

    if rel == 'res/msi/preprocess.py':
        # Change only CLI defaults. Keep regexes that recognize upstream legal
        # attribution in License.rtf so the processor remains compatible.
        text = re.sub(
            r'("--app-name",\s*type=str,\s*default=")RustDesk("\s*,)',
            lambda m: m.group(1) + APP_DISPLAY_NAME + m.group(2),
            text,
            count=1,
        )
        text = re.sub(
            r'(default=")Purslane Tech Pte\. Ltd\.("\s*,)',
            lambda m: m.group(1) + COPYRIGHT_OWNER + m.group(2),
            text,
            count=1,
        )

    if rel == 'res/msi/Package/Language/Package.en-us.wxl':
        # IDs are stable WiX lookup keys; only Value= is public text.
        def wxl_repl(match: re.Match[str]) -> str:
            return match.group(1) + _replace_public_brand_token_v40(match.group(2)) + match.group(3)
        text = re.sub(r'(<String\b[^>]*\bValue=")([^"]*)(")', wxl_repl, text)

    # Harden metadata variants that may use the newer upstream company spelling.
    if rel in {'Cargo.toml', 'libs/portable/Cargo.toml', 'flutter/macos/Runner/Configs/AppInfo.xcconfig', 'flutter/windows/runner/Runner.rc'}:
        text = re.sub(r'Purslane Tech Pte\. Ltd\.?', COPYRIGHT_OWNER, text)
        text = re.sub(r'Purslane Ltd\.?', COPYRIGHT_OWNER, text)
    return text
...
def patch_text(rel: str, text: str, args: argparse.Namespace) -> str:  # type: ignore[override]
    text = _PRE_V40_PATCH_TEXT(rel, text, args)
    text = patch_language_values_v40(rel, text)
    text = patch_public_vendor_and_literals_v40(rel, text)
    return text


def _v40_public_brand_risks(target: Path) -> list[tuple[str, str]]:
    issues: list[tuple[str, str]] = []
    lang_rs = target / 'src/lang.rs'
    if lang_rs.is_file():
        t = normalize_lf(lang_rs.read_text(encoding='utf-8', errors='ignore'))
        if re.search(r'&&\s*name\s*!=\s*"powered_by_me"', t):
            issues.append(('src/lang.rs', 'V40: powered_by_me ainda está excluído da substituição de marca'))

    for p in sorted((target / 'src/lang').glob('*.rs')) if (target / 'src/lang').is_dir() else []:
        rel = p.relative_to(target).as_posix()
        t = p.read_text(encoding='utf-8', errors='ignore')
        for m in _LANG_TUPLE_RE_V40.finditer(t):
            value = m.group(2)
            if re.search(r'(?i)rustdesk', value):
                issues.append((rel, 'V40: valor de tradução público ainda contém RustDesk'))
                break
    ...
    for rel in sorted(public_vendor_files):
        p = target / rel
        if not p.is_file():
            continue
        t = p.read_text(encoding='utf-8', errors='ignore')
        if re.search(r'Purslane(?: Tech Pte\.)? Ltd\.?', t, flags=re.I):
            issues.append((rel, 'V40: metadado/UI público ainda contém Purslane'))

    tab = target / 'flutter/lib/desktop/widgets/tabbar_widget.dart'
    if tab.is_file() and re.search(r'child:\s*const\s+Text\(\s*"RustDesk"', tab.read_text(encoding='utf-8', errors='ignore')):
        issues.append(('flutter/lib/desktop/widgets/tabbar_widget.dart', 'V40: título público ainda está hardcoded como RustDesk'))

    wxl = target / 'res/msi/Package/Language/Package.en-us.wxl'
    if wxl.is_file():
        t = wxl.read_text(encoding='utf-8', errors='ignore')
        for m in re.finditer(r'<String\b[^>]*\bValue="([^"]*)"', t):
            if re.search(r'(?i)rustdesk', m.group(1)):
                issues.append(('res/msi/Package/Language/Package.en-us.wxl', 'V40: texto público MSI ainda contém RustDesk'))
                break
    return issues


def validate_build_safety(target: Path, report: Dict[str, Any]) -> None:  # type: ignore[override]
    _PRE_V40_VALIDATE_BUILD_SAFETY(target, report)
    for rel, msg in _v40_public_brand_risks(target):
        report['pending'].append({'file': rel, 'message': msg})
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--log-file", default=None, help="Arquivo de log detalhado. Padrão: <target>/rebrand_v40.log")
...
def main() -> int:
    ...
    # V40: idiomas são descobertos dinamicamente. Novas versões do upstream podem
    # adicionar arquivos em src/lang sem atualizar nossa allowlist estática; em
    # runtime/full eles também precisam receber apenas o brand nos valores públicos.
    if args.profile in {"runtime", "full"}:
        lang_dir = target / "src/lang"
        if lang_dir.is_dir():
            candidates = sorted(set(candidates) | {
                p.relative_to(target).as_posix() for p in lang_dir.glob("*.rs") if p.is_file()
            })
