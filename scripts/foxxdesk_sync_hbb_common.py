...
SCRIPT_VERSION = "foxxdesk-hbb-sync-v11-exact-version-2026-09-06"
...
def expected_commit(root: Path, cfg: dict) -> tuple[str, str]:
    ...
    policy = str(upstream.get("hbb_common_policy", "exact-version")).strip().lower()
    if policy != "exact-version":
        raise SyncError(f"Política hbb_common não suportada: {policy!r}; use 'exact-version'")
    if bool(upstream.get("allow_main_fallback", False)):
        raise SyncError("allow_main_fallback=true é proibido na V11; hbb_common/main nunca é usado")
    ...
    if configured_ref.lower() in {"main", "master", "latest"}:
        raise SyncError("rustdesk_ref não pode ser main/master/latest em exact-version; use auto, uma versão ou uma tag exata")
...
def main() -> int:
    ...
    try:
        ...
        print(f"[hbb] RustDesk detectado: {version} | commit esperado: {expected}")
