...
SCRIPT_VERSION = "foxxdesk-build-compat-v2-workflow-expression-safe-2026-09-06"
...
def normalize_workflow_if_literals(text: str) -> str:
    """Normalize YAML/Python boolean spellings inside GitHub `if:` expressions.

    Some source snapshots were previously serialized through YAML and contain
    `if: False`. GitHub expressions accept lowercase `false`, not Python `False`.
    A disabled condition remains disabled instead of being combined with gates.
    """
    out: list[str] = []
    for line in text.splitlines():
        ...
        # `false && ...` is equivalent to false and avoids unnecessary parsing.
        if re.match(r'^false\s*&&', expr):
            expr = 'false'
        elif re.match(r'^true\s*&&\s*', expr):
            expr = re.sub(r'^true\s*&&\s*', '', expr, count=1)
        out.append(prefix + expr + suffix)
    return '\n'.join(out) + ('\n' if text.endswith('\n') else '')


def patch_flutter_build_workflow(text: str) -> str:
    text = normalize_workflow_if_literals(text)
    ...
    return normalize_workflow_if_literals("\n".join(lines).rstrip() + "\n")
...
def check_invariants(root: Path, cfg: dict) -> list[str]:
    ...
    if not flutter.is_file():
        ...
        t = read(flutter)
        if re.search(r'(?m)^\s*if:\s*(?:False|True)\b', t):
            errors.append('flutter-build.yml: expressão if contém booleano Python False/True; use false/true')
